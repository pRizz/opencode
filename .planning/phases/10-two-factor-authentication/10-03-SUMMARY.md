...
plan: 03
subsystem: auth
tags: [jwt, tokens, device-trust, totp]

dependency-graph:
  requires: [10-01]
  provides: [device-trust-tokens, totp-tokens]
  affects: [10-04, 10-05]
...
JWT-based token modules for device trust and TOTP intermediate tokens using jose library.
...
2. **IP Binding**: TOTP tokens optionally bind to IP address for added security
3. **Stateless Design**: All user info embedded in token - no server state needed
4. **Short Expiration**: TOTP tokens meant for 5-minute window (configurable)
...
| 062a3b5a3 | feat | Add TOTP token module                |
...
Ready for 10-04 (TOTP login routes) and 10-05 (device trust routes):
